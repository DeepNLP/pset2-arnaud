...
def softmax(x):
  ...
  ### YOUR CODE HERE
  rescaled_x = x - tf.reduce_max(x, 1, keep_dims=True)
  exp_x = tf.exp(rescaled_x)
  sum_exp_x = tf.reduce_sum(exp_x, 1, keep_dims=True)
  out = exp_x/sum_exp_x
  ### END YOUR CODE
  return out
...
def cross_entropy_loss(y, yhat):
  ...
  out = -tf.reduce_sum(tf.to_float(y)*log_yhat)
  ### END YOUR CODE
  return out
